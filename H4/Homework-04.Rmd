...
newckm<-newckm %>% mutate(begin = (ckm_nodes$adoption_date[doctor]==month))
newckm<-newckm %>% mutate(before = (ckm_nodes$adoption_date[doctor]<month))
newckm<-newckm %>% group_by(doctor,month) %>%
    mutate(constrbefore=
                  sum((ckm_network[,doctor]==1)&(ckm_nodes$adoption_date<month))) %>%
    ungroup()
newckm<-newckm %>% group_by(doctor,month) %>%
    mutate(conbefore=
                  sum((ckm_network[,doctor]==1)&(ckm_nodes$adoption_date<=month))) %>%
