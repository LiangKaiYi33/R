...
&emsp;&emsp;
根据之前的duration~waiting图，可以发现根据duration，数据大致可以分为两组，一组为duration$\geq3$的情况，另一种为duration$<$3的情况，在这里我们选取duration$\geq$3的情况。在绘制图像之前需要先用dplyr::filter将符合条件的数据提取出来，这组数据称为geyser3。并且建立一个新的数据wtds3来存放每个waiting值得频率。结果如下图，其中黑色的线是密度函数。
...
&emsp;&emsp;
取初值p为duration$\leq$3时的密度和，$\mu_1$为2，$\sigma_1$为0.5，$\mu_2$为4，$\sigma_2$为0.5，数据用geyser$duration。拟合出的参数如下。
```{r,echo=F,message=F}
