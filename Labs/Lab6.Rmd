...
```{r}
ckm_nodes %>% ggplot()+
  geom_bar(aes(x=adoption_date))
ckm_nodes %>% ggplot()+
  stat_ecdf(aes(x=adoption_date))
```
...
```{r}
vec2<-(ckm_nodes$adoption_date<=2)
which(vec2)
```
...
```{r}
vec3<-(ckm_nodes$adoption_date>14)
which(vec3)
```
...
any(ckm_network!=0&&ckm_network!=1)
ckm_network<-ckm_network[vec1,] %>% select(vec1)
```
...
```{r}
ckm_network$num<-apply(ckm_network,1,sum)
ckm_network$num[41]
```
...
```{r}
find<-c()
for(i in 1:dim(ckm_network)[1]){
  if(ckm_network[i,37]==1&ckm_nodes$adoption_date[i]<=5){
    find<-c(find,TRUE)
  }
  else{
    find<-c(find,FALSE)
