...
**解**：
```{r}
plot(Median_house_value~Built_2005_or_later,data=ca_pa1,pch=16,cex=0.3)
```
...
**解**：
```{r}
xyplot(Median_house_value~Built_2005_or_later|STATEFP,data=ca_pa1,pch=16,cex=0.3,auto.key=list(columns=2),subset=STATEFP%in%c("06","42"))
```
...
**解**：
```{r}
plot(Vacancy_rates~Median_house_value,data=ca_pa2,pch=16,cex=0.3)
```
...
**解**：
```{r}
xyplot(Vacancy_rates~Median_house_value|STATEFP,data=ca_pa1,pch=16,cex=0.3,auto.key=list(columns=2))
```
